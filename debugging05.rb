...
# we used map instead of select. map would make an array of [false, true, false true, false, false, true], and select would make an array only of the values which evaluate to true.

# my example map array wasn't right

=begin


What is the return value of the block when n is an even number? What about when n is odd?

Array#map is used for transformation. It returns a new array containing the return value of the block for each iteration. The expression n if n.even? returns the integer n when it is even, and returns nil if it is odd. Thus, with map, we get an array that looks like this: [nil, 2, nil, 6, nil, nil, 8].
